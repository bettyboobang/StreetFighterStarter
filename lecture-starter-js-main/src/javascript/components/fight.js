...
export async function fight(firstFighter, secondFighter) {
    return new Promise(resolve => {//we return a new Promise that will resolve with the winner of the fight once the fight is over.
        ...
        function handleKeyDown(event) {//event handler for keydown events
            ...
            const isP1Blocking = pressedKeys[controls.PlayerOneBlock];//checking if the block key for player 1 is currently pressed
            const isP2Blocking = pressedKeys[controls.PlayerTwoBlock];
            if (isComboPressed(controls.PlayerOneCriticalHitCombination) && p1CritAvailable) {
                p1CritAvailable = false;
                const damage = 2 * player1.attack;
                player2.currentHealth -= damage;
                updateHealthBar(player2, p2HealthBar);
                playHitAnimation('right');
                setTimeout(() => { p1CritAvailable = true; }, 10000);
                checkWinner();
                return;
            }
            if (isComboPressed(controls.PlayerTwoCriticalHitCombination) && p2CritAvailable) {
                p2CritAvailable = false;
                const damage = 2 * player2.attack;
                player1.currentHealth -= damage;
                updateHealthBar(player1, p1HealthBar);
                playHitAnimation('left');
                setTimeout(() => { p2CritAvailable = true; }, 10000);
                checkWinner();
                return;
            }
            if (event.code === controls.PlayerOneAttack && !isP1Blocking) {
                const damage = isP2Blocking ? getDamage(player1, player2) : getHitPower(player1);
                if (damage > 0) {
                    player2.currentHealth -= damage;
                    updateHealthBar(player2, p2HealthBar);
                    playHitAnimation('right'); 
            ...
            // Гравець 2 робить ЗВИЧАЙНИЙ удар
            if (event.code === controls.PlayerTwoAttack && !isP2Blocking) {
                const damage = isP1Blocking ? getDamage(player2, player1) : getHitPower(player2);
                // Запускаємо анімацію, тільки якщо удар завдав шкоди
                if (damage > 0) {
                    player1.currentHealth -= damage;
                    updateHealthBar(player1, p1HealthBar);
                    playHitAnimation('left');
...
function playHitAnimation(position) {
    const selector = position === 'right' 
        ? '.arena___right-fighter img' 
        : '.arena___fighter img'; 
        
    const fighterImage = document.querySelector(selector);
    if (fighterImage) {
        fighterImage.classList.add('fighter___hit');
        setTimeout(() => {
            fighterImage.classList.remove('fighter___hit');
        }, 300);
    }
}